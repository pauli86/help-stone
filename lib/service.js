...
const Service = {
        ...
        .then(ret=>{
            ...
            if(ret.result===1){
                ...
                [{
                    text:'확인',
                    onPress:()=>{
                        console.log('onPress 확인');
                        Service.goto('login');
                    }
                }]);
            }else{
                Alert.alert('아이디 찾기',msg);
            }
        })
        .catch(e=>console.log(e))
    },
    _resetPW:(id,name,stuNo,pw)=>{
        fetch(apiURL+'/user/resetPW',{
            method:'post',
            headers:{
                Accept : 'application/json',
                'Content-Type':'application/json'
    ...
    addTask:(title,desc)=>{
        ...
        if(!title){
            return Alert.alert('TASK 생성 오류','제목을 입력하세요');
        }
